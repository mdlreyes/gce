...
def plotting_compare(model, atomic, title1, model2=[], atomic2=[], title2='', plot=False, 
    ...
    # Open data
    elem_data, delem_data = getdata(galaxy='Scl', c=True, ba=True)
    feh_obs = elem_data[0,:]
    mgfe_obs = elem_data[1,:]
    sife_obs = elem_data[2,:]
    cafe_obs = elem_data[3,:]
    cfe_obs = elem_data[4,:]
    bafe_obs = elem_data[5,:]
    #print(bafe_obs)
    ...
    print('test', np.arange(len(labels)), range(len(labels)))
    for i in np.arange(len(labels)):
        if eu_estimate == True and i == 2:
            ...
            if i == 3:
                goodidx = np.where((feh_obs > -990) & (cfe_obs > -990))[0]
                print(len(feh_obs), len(cfe_obs), len(goodidx))
                axs[i].scatter(feh_obs[goodidx], cfe_obs[goodidx], c='r', s=1/delem_data[4,goodidx])
            if i == 5:
                goodidx = np.where((feh_obs > -990) & (mgfe_obs > -990))[0]
                print(len(feh_obs), len(mgfe_obs), len(goodidx))
                axs[i].scatter(feh_obs[goodidx], mgfe_obs[goodidx], c='r', s=1/delem_data[1,goodidx])
            if i == 6:
                goodidx = np.where((feh_obs > -990) & (sife_obs > -990))[0]
                print(len(feh_obs), len(sife_obs), len(goodidx))
                axs[i].scatter(feh_obs[goodidx], sife_obs[goodidx], c='r', s=1/delem_data[2,goodidx])
            if i == 7:
                goodidx = np.where((feh_obs > -990) & (cafe_obs > -990))[0]
                print(len(feh_obs), len(cafe_obs), len(goodidx))
                axs[i].scatter(feh_obs[goodidx], cafe_obs[goodidx], c='r', s=1/delem_data[3,goodidx])
            if i == 8:
                #print('made it here')
                goodidx = np.where((feh_obs > -990) & (bafe_obs > -990))[0]
                print(len(feh_obs), len(bafe_obs), len(goodidx))
                #print(bafe_obs[goodidx])
                axs[i+1].scatter(feh_obs[goodidx], bafe_obs[goodidx], c='r', s=1/delem_data[5,goodidx])
                #print(bafe_obs, delem_data[5,:])
