...
scl_rampressure = [0.40324345287403596,0.31993145962170993,4.678875833236568,0.4311305388839332,0.901133475075236,0.,0.5687645229241343,1.2899619752803073,0.8487435966713881,0.2857591805747674,1.5867499800816725,0.9139277884487471,3-0.013695390962180884,2.761601093904625]
scl_iadtd_maoz17 = [0.5164266933312606,0.22668646095205333,5.083938866111591,0.6708444072138402,0.7705726151153531,0.,0.8952553665964144,1.3892756558045938,0.47734137612923366,0.14861562077682866,0.435341510493796,0.33493152648882035,3-0.013300279965722219, 0.]
scl_iadtd_himin = [0.12655011964708354,0.48083816401816953,4.774355073300842,1.0142068952827288,0.13937182878786417,0.,0.8931868300415441,1.548247398565755,0.40718356024183644,0.037801676437251096,0.5965616973892593,0.8185482795511301,3-1.3007176610686921, 0.]
scl_iadtd_lomin = [0.5671755376691066,0.29287428128668863,5.015868834444396,0.30612159610634737,1.0034407937884338,0.,0.44599230385432126,1.161552777383641,1.2779361777765668,0.43394265803765714,1.326460414915983,0.9806089731602307,3-0.0059330597053563775, 0.]
scl_iadtd_medhimin = [0.2581362038956129,0.3671543880935386,4.884509919096489,0.519627246584796,0.6921089016677752,0.,0.667125036335381,1.4048235865635883,0.6442822761890015,0.17279690215969257,1.076567643613428,0.5756914070867104,3-0.0789755064127836, 0.]
scl_iadtd_loindex = [0.31309403734878677,0.32698844029546187,5.119860962340789,0.5074794913085319,0.7677725611456582,0.,0.7747072609145225,1.3579266977743019,0.6409945773107304,0.24639512831333843,0.8835860105097602,0.5557520537168783,3-0.023556275510243575, 0.]
scl_iadtd_hiindex = [0.5446693466306317,0.3092340505074539,4.662881112688744,0.6610461169621856,0.6648004259776421,0.,0.434048932393723,1.2372641358088885,1.21868854143266,0.30455907377622926,2.5503064633438433,0.9921019155833941,3-0.00552116094663595, 0.]
...
def compare_sfh(models, title, fiducialtitle='Fiducial'):
    ...
    for i, modelname in enumerate(models):
        ...
        if modelname=='fiducial':
            ...
            plt.plot(model['t'],model['mdot']/1e5, color=colors[i], ls=linestyles[i], lw=2, label=titles[modelname]+r', $\Delta(\mathrm{AIC})=$'+' '+str(int(llnames[modelname]-ll)))
...
def compare_yields(plottype, feh_denom=True):
    ...
    if plottype=='IaSN':
        ...
        for idx, label in enumerate(labels):
            ...
            for line_idx, iamodel in enumerate(elemdicts[idx]):
                if label=='Mn':
                    model, atomic, ll = gce.runmodel(scl_final, plot=False, title="Sculptor dSph", empirical=True, empiricalfit=True, feh_denom=True, delay=False, reioniz=False, mgenhance=False, nomgas0=True, mn=elemdicts[idx][iamodel])
                elif label=='Ni':
                    model, atomic, ll = gce.runmodel(scl_final, plot=False, title="Sculptor dSph", empirical=True, empiricalfit=True, feh_denom=True, delay=False, reioniz=False, mgenhance=False, nomgas0=True, ni=elemdicts[idx][iamodel])
    ...
    if plottype=='rprocess':
        ...
        for idx, label in enumerate(labels):
            ...
            for line_idx, keyword in enumerate(rprocesskeywords):
                model, atomic, ll = gce.runmodel(scl_final, plot=False, title="Sculptor dSph", empirical=True, empiricalfit=True, feh_denom=True, delay=False, reioniz=False, mgenhance=False, nomgas0=True, rprocess=keyword)
    ...
    if plottype=='rprocess_bestfit':
        ...
        for idx, label in enumerate(labels):
            ...
            for line_idx, keyword in enumerate(rprocesskeywords):
                model, atomic, ll = gce.runmodel(scl_final, plot=False, title="Sculptor dSph", empirical=True, empiricalfit=True, feh_denom=True, delay=False, reioniz=False, mgenhance=False, nomgas0=True, rprocess=keyword, specialrprocess=specialkeywords[line_idx])
